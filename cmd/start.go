		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if hasRun && run.CostCapped {
				...
				if rollup.SkippedFiles > 0 {
					_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "warning: %d cost.json file(s) unreadable; prior spend may under-count current actual spend\n", rollup.SkippedFiles)
				}
			...
			if err != nil {
				if batch.IsMissingBatchError(err) {
					...
					_, _ = fmt.Fprintf(cmd.OutOrStdout(), "No active batch: prior run %q was orphaned and has been archived.\nRun \"springfield plan\" to start fresh.\n", run.ActiveBatchID)
					return nil
				}
			...
			if logErr != nil {
				_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "warning: failed to open log file: %v\n", logErr)
			} else {
				defer closeLog()
			}
			...
			if perPlan {
				_, _ = fmt.Fprintf(w, "Branch mode: per-plan (base: %s)\n", batchBase)
			}
			...
			if abErr != nil {
				...
				if !hadPriorAutoBranch && run.AutoBranchName != "" {
					...
					if werr := batch.WriteRun(root, run); werr != nil {
						_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "warning: roll back auto-branch state: %v\n", werr)
					}
				}
			...
			if !noKeepAwake && loaded.Config.KeepAwakeEnabled() {
				...
				if wlErr != nil {
					_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "warning: sleep prevention unavailable: %v\n", wlErr)
				} else {
					_, _ = fmt.Fprintf(w, "Sleep prevention: active\n")
			...
			if errors.Is(execErr, context.Canceled) {
				...
				_, _ = fmt.Fprintf(w, "Status: interrupted\n")
				_, _ = fmt.Fprintf(w, "Info: rerun \"springfield start\" to resume\n")
			...
			if result.CostCapped {
				...
				autoBranchOutcome = autobranch.OutcomeInterrupted
				_, _ = fmt.Fprintf(w, "Status: cost-capped\n")
				_, _ = fmt.Fprintf(w, "Est. API cost: $%.2f (cap: $%.2f)\n", result.SpendUSD, costCap)
				...
				_, _ = fmt.Fprintf(w, "Info: rerun with --cost-cap $Y to continue (Y > current spend) or remove claude from agent_priority to reduce spend\n")
			...
			if project, projErr := conductor.LoadProject(root); projErr != nil {
				_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "warning: load project for finalize: %v; archiving without per-plan enrichment\n", projErr)
				if archiveErr := batch.ArchiveBatchNormalizedWithMode(root, b, "completed", archiveRollup, run.BatchMode); archiveErr != nil {
					_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "warning: archive completed batch %q: %v\n", b.ID, archiveErr)
				}
			...
			_, _ = fmt.Fprintf(w, "Status: completed\n")
			if archiveRollup != nil && archiveRollup.Iterations > 0 {
				_, _ = fmt.Fprintln(w, formatTotalSpendLine(*archiveRollup))
			}
			return nil
		},
	}
...
func buildNotifier(cmd *cobra.Command, rootDir string) notify.Notifier {
	...
	if err != nil {
		_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "warning: notifications disabled, load springfield.local.toml: %v\n", err)
		return notify.Nop{}
	}
...
func runBatchWithContext(ctx context.Context, root string, run *batch.Run, b batch.Batch, progress io.Writer, logPath string, costCap float64, perPlan bool, batchBase string, maxParallel int) (BatchRunResult, error) {
	...
	execRes, execErr := batchexec.Execute(ctx, batchexec.Input{
		...
		OnPhaseStart: func(index int, phase batch.Phase, parallel bool, cap int) {
			if parallel {
				_, _ = fmt.Fprintf(progress, "Phase %d/%d (parallel): up to %d of %d plans concurrently\n",
...
func (r *batchPlanRunner) RunPlan(ctx context.Context, planID string, info batchexec.RunInfo) batchexec.Outcome {
	...
	if st, ok := r.project.ReadPlan(planID); ok && st.Status == conductor.StatusCompleted {
		_, _ = fmt.Fprintf(progress, "Plan: %s\n", planID)
		_, _ = fmt.Fprintf(progress, "Status: resuming integration\n")
	...
	if res.CostCapped {
		_, _ = fmt.Fprintf(progress, "Plan: %s\n", res.PlanID)
		_, _ = fmt.Fprintf(progress, "Status: cost-capped (%s)\n", res.Reason)
	...
	if res.Err != nil {
		...
		if needsHuman {
			_, _ = fmt.Fprintf(progress, "Status: needs human review (%s)\n", res.Reason)
		} else {
			_, _ = fmt.Fprintf(progress, "Status: failed (%s)\n", res.Reason)
		}
		_, _ = fmt.Fprintf(progress, "Error: %s\n", res.Err.Error())
	...
	_, _ = fmt.Fprintf(progress, "Plan: %s\n", res.PlanID)
	_, _ = fmt.Fprintf(progress, "Status: completed\n")
	if res.EvidencePath != "" {
		_, _ = fmt.Fprintf(progress, "Evidence: %s\n", res.EvidencePath)
	}
...
func teardownHookFor(setup config.SetupConfig, ports config.PortsConfig, project *conductor.Project, planID, sourceRoot string, progress io.Writer) func(string) {
	...
	return func(worktreePath string) {
		...
		_, _ = fmt.Fprintf(progress, "teardown %s: %s\n", planID, req.Command)
		res := worktreesetup.Run(context.Background(), req)
		switch {
		case res.Err != nil:
			_, _ = fmt.Fprintf(progress, "teardown %s: WARN could not launch (cleanup continues): %v\n", planID, res.Err)
		case res.ExitCode != 0:
			detail := "exit " + strconv.Itoa(res.ExitCode)
			if res.TimedOut {
				detail += " (timed out)"
			}
			_, _ = fmt.Fprintf(progress, "teardown %s: WARN command failed %s (cleanup continues)\n", planID, detail)
		}
	}
}
...
func tryRunSinglePlanUnit(cmd *cobra.Command, root string, loaded config.Loaded, noKeepAwake bool, costCap float64) (bool, error) {
	...
	saveQueueState := func() {
		if err := project.SaveState(); err != nil {
			_, _ = fmt.Fprintf(w, "warning: failed to persist queue state: %v\n", err)
		}
	}
	...
	for {
		if err := ctx.Err(); err != nil {
			...
			saveQueueState()
			_, _ = fmt.Fprintln(w, "Queue stopped: interrupted by signal")
			return true, nil
		}
	...
	_, _ = fmt.Fprintf(w, "Queue completed: %d of %d plans integrated\n", completed, total)
	return true, nil
}
...
func runOnePlan(ctx context.Context, w io.Writer, project *conductor.Project, root, worktreeBase string, agentIDs []agents.ID, loaded config.Loaded, local config.LocalConfig, registry agents.Registry, costCap float64) error {
	...
	if planID, ok := nextNonIntegratedCompletedPlan(project); ok {
		...
		if enforceProtected {
			if prior := project.State.Plans[planID]; prior != nil && planrun.IsProtectedBase(prior.BaseRef) {
				...
				_, _ = fmt.Fprintf(w, "Status: failed (preflight-protected-base)\n")
	...
	if res.CostCapped {
		...
		_, _ = fmt.Fprintf(w, "Info: rerun with --cost-cap $Y to continue (Y > current spend)\n")
		...
		if writeErr := batch.WriteRun(root, existing); writeErr != nil {
			_, _ = fmt.Fprintf(w, "warning: persist cost-capped state: %v\n", writeErr)
		}
	...
	if res.Err != nil {
		...
		if res.Context.WorktreeRoot != "" {
			_, _ = fmt.Fprintf(w, "Worktree: %s (branch %s, base %s @ %s)\n",
		...
		if res.Status == conductor.StatusNeedsHuman {
			_, _ = fmt.Fprintf(w, "Status: needs human review (%s)\n", res.Reason)
		} else {
...
func runMergeIntegrationOnly(w io.Writer, project *conductor.Project, root, worktreeBase, planID string, setup config.SetupConfig, ports config.PortsConfig) (bool, error) {
	_, _ = fmt.Fprintf(w, "Plan: %s (re-running merge integration)\n", planID)
	mergeRes := planmerge.Integrate(planmerge.IntegrateInput{
		Project:      project,
		PlanID:       planID,
		ControlRoot:  root,
...
func renderMergeOutcome(w io.Writer, res planmerge.IntegrateResult) {
	...
	if m.Reason == "" {
		...
		_, _ = fmt.Fprintf(w, "Merge: %s (%s)\n", m.Status, m.Reason)
	}
	if m.TargetRef != "" {
		_, _ = fmt.Fprintf(w, "  target: %s\n", m.TargetRef)
	}
	if m.TargetHead != "" {
		_, _ = fmt.Fprintf(w, "  target head: %s\n", shortSHA(m.TargetHead))
	}
	if m.PostMergeHead != "" {
		_, _ = fmt.Fprintf(w, "  post-merge head: %s\n", shortSHA(m.PostMergeHead))
	}
	if m.WorktreePath != "" && m.Status != conductor.MergeSucceeded {
		_, _ = fmt.Fprintf(w, "  merge worktree (preserved): %s\n", m.WorktreePath)
	}
	if m.Error != "" {
