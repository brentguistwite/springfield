...
You are reviewing a completed unit of work before it is merged into the feature branch. Emit EXACTLY ONE verdict marker as the ONLY content on its own line — no surrounding prose, no quoting, NO leading or trailing whitespace (the marker must start in column 0). The scanner ignores verdict-shaped text that appears mid-line, indented, or inside quoted code — if your marker is on the same line as anything else (including quoted snippets from the diff) it will be ignored and your review will be treated as verdict-less.
