...
Project-level agent execution settings also live in `springfield.toml`. Example:
...
- Safe defaults remain the default. If you omit these fields, Springfield does not force an explicit Claude permission mode or Codex sandbox/approval mode.
...
- Truly hands-off Ralph or Conductor runs require a mode with sufficient permissions for the work being attempted. Otherwise runs may pause for approval or fail at permission boundaries.
