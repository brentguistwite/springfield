...
func TestCodexValidatorAcceptsOptionalMCPAuthFailureWhenWorkCompleted(t *testing.T) {
	...
	result := coreexec.Result{
		...
		Events: []coreexec.Event{
			...
			{Type: coreexec.EventStdout, Data: `{"type":"item.completed","item":{"id":"item_1","type":"command_execution","command":"sed -n '1,220p' ~/.codex/superpowers/skills/using-superpowers/SKILL.md","aggregated_output":"...","exit_code":0,"status":"completed"}}`, Time: time.Now()},
